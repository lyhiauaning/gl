  ...
  if [[ "$1" == "--all" || "$1" == "-a" ]];then
    ...
    do
      ...
      if expr "$i" : ".*\.cmt\.xml" &> /dev/null ;then
        python $PREFIX/etc/gl/danmaku2ass -o "${i%.cmt.xml*}.ass" -s ${videowidth}x${videoheight} -fn "Microsoft Yahei" -fs 48 -a 0.8 -dm 14 -ds 6 "$i"
      elif expr "$i" : ".*\.xml" &> /dev/null ;then
        python $PREFIX/etc/gl/danmaku2ass -o "${i%.xml*}.ass" -s ${videowidth}x${videoheight} -fn "Microsoft Yahei" -fs 48 -a 0.8 -dm 14 -ds 6 "$i"
      fi
    done
  fi
else
